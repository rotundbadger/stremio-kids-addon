...
module.exports = async function catalogHandler(args) {
    try {
        ...
        if (!parsed) {
            console.log(`[catalog] failed to parse catalog ID: ${id}`);
            return { metas: [] };
        }
...
async function handleDiscover(type, page, genre, ageRange) {
    ...
    console.log(`[catalog] discover returned ${results.length} results for ${type} age ${ageRange}`);
    ...
    const metas = await Promise.all(
        results.map(async (item) => {
            ...
            if (!imdbId) {
                console.log(`[catalog] no IMDB ID for TMDB ${item.id} (${item.title || item.name})`);
                return null;
            }
    ...
    console.log(`[catalog] returning ${filtered.length} metas (from ${results.length} results)`);
    return filtered;
}
